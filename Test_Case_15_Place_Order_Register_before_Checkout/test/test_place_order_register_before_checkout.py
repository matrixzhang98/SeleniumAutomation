...
test_data_path = os.path.abspath(config.TEST_CASE_15_REGISTER_BEFORE_CHECKOUT_DATA)

with open(test_data_path) as f:
    test_data = json.load(f)
    test_list = test_data["data"]

@allure.feature("place order register before checkout")
@allure.story("End to End place order register before checkout")
@allure.title("Test E2E place order register before checkout")
@pytest.mark.parametrize("test_data_list", test_list)
def test_place_order_register_before_checkout(pages, test_data_list):
    ...
    with allure.step("Step 14: Enter payment details: Name on Card, Card Number, CVC, Expiration date"):
        pages.payment.enter_payment_details(test_data_list)

    with allure.step("Step 15: Click 'Pay and Confirm Order' button"):
    ...
    with allure.step("Step 18: Verify 'ACCOUNT DELETED!' and click 'Continue' button"):
